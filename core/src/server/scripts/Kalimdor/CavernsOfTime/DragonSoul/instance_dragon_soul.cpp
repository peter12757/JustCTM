    ...
    struct instance_dragonsoul_InstanceMapScript : public InstanceScript
    {
        ...
        void changeEntrancePortal()
        {
            ...
            for (auto itr : entrancePortals)
                if (Creature *portal = GetCreature(itr))
            ...
            if (currentPortal != 0)
                if (Creature *portal = GetCreature(currentPortal))
        ...
        void OnCreatureCreate(Creature *creature) override
        {
            ...
            else if (raidNerfData == RAID_NERF_ACTIVE)
            {
                ...
                //todo
                creature->CastSpell(creature, SPELL_PRESENCE_OF_THE_DRAGON_SOUL, CastSpellExtraArgs().AddSpellBP0(currentNormalAndHcNerfAmount).AddSpellBP1(currentNormalAndHcNerfAmount));
            ...
            case NPC_EARTHEN_SOLDIER:
                ...
                if (IsDone(DATA_MORCHOK) || LfrId == LFR_PART_TWO)
                {
                    ...
                    TC_LOG_DEBUG("INSTANCE_DRAGON_SOUL","IsDone(DATA_MORCHOK) || LfrId == LFR_PART_TWO");
                }
            ...
            case NPC_MORCHOK:
                ...
                {
                    // creature->SetPhaseMask(0x2, true);
                    TC_LOG_DEBUG("INSTANCE_DRAGON_SOUL", "LfrId == LFR_PART_TWO");
                }
                break;
            default:
                break;
                    ...
                    if (state == DONE)
                {
                    for (auto itr : npcGroups)
                        if (itr.first == DATA_MORCHOK)
                            if (Creature *trashMob = GetCreature(itr.second))
                            {
                                // creature->SetPhaseMask(0x2, true);
                                TC_LOG_DEBUG("INSTANCE_DRAGON_SOUL", "Creature *trashMob = GetCreature(itr.second)");
                            }

                    for (auto itr : npcPortals)
                        if (itr.first == DATA_MORCHOK)
                            if (Creature *portal = GetCreature(itr.second))
                                portal->SetVisible(true);

                    for (uint64 trashGUID : wingTrash)
                        if (Creature *trashMob = GetCreature(trashGUID))
                        {
                            // creature->SetPhaseMask(0x2, true);
                            TC_LOG_DEBUG("INSTANCE_DRAGON_SOUL", "Creature *trashMob = GetCreature(trashGUID)");
                        }
